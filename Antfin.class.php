...
class Antfin{
    ...
    public function getCode(){
        $url = "https://openauth.alipay.com/oauth2/appToAppAuth.htm?app_id=".$this->alipaySet['appId']."&redirect_uri=".$this->alipaySet['redirect_uri'];
        Header("Location:$url");
        //打印获得的数据
    }
    /*
    ...
    public function getToken($code){
        ...
        $ss['apiMethodName'] = "alipay.offline.market.shop.batchquery";
        $re = $this->execute($ss);
        return $re;
        //打印获得的数据
    }
     ...
     * @return array token
     */
    public function getAccToken_s($code,$refresh_token){
        if($refresh_token){
        ...
        $ss['content']['biz_content'] = json_encode($data);
        $ss['apiMethodName'] = "alipay.open.auth.token.app";
        $re = $this->execute($ss);
        return $re;
    }
    /**
     * @return array token
     */
    public function getAccToken($merchant_pid){
        if(empty($merchant_pid)){
            return array('err'=>-1,'msg'=>"商家ID为空!");
        }else{
            $where['user_id']=$merchant_pid;
            $merchant = M('merchant')->where($where)->find();
            if(empty($merchant['app_auth_token'])){
                return array('err'=>-1,'msg'=>"商家未授权!");
            }else if($merchant['app_auth_token_time']+intval($merchant['expires_in']) <= time()){
                //return array('err'=>-1,'msg'=>"授权token已过期!");
                $this->getAccToken_s($merchant_pid);
            }else{
                return $merchant['app_auth_token'];
            }
        }
    }
    /*
	 * 获取code
	 */
    public function getUserCode($redirect_uri){
        $url = "https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id=".$this->alipaySet['appId']."&scope=auth_userinfo&redirect_uri=".$redirect_uri;
        Header("Location:$url");
        //打印获得的数据
    }
    /*
    ...
    public function getUserToken($data){
        $ss['content']['grant_type'] = $data['grant_type'];
        $ss['content']['code'] = $data['code'];
        $ss['apiMethodName'] = "alipay.system.oauth.token";
        $re = $this->execute($ss);
        return $re['alipay_system_oauth_token_response'];
        //打印获得的数据
    }
    /*
	 * 用户通过token获取信息
    ...
    public function getUserInfo($token){
        $ss['apiMethodName'] = "alipay.user.info.share";
        $re = $this->execute($ss,$token);
        return $re['alipay_user_info_share_response'];
        //打印获得的数据
    }
    /*
	 * 用户通过token获取信息
	 */
    public function getuserUserInfo($token){
        $ss['apiMethodName'] = "alipay.user.userinfo.share";
        $re = $this->execute($ss,$token);
        return $re['alipay_user_userinfo_share_response'];
        //打印获得的数据
    }
    /**
     * @return 门店列表
     */
    public function shopBatchquery($data){
        $ss['content']['biz_content'] = json_encode($data);
        $ss['apiMethodName'] = "alipay.offline.market.shop.batchquery";
        $re = $this->execute($ss);
        return $re['alipay_offline_market_shop_batchquery_response'];
    }
    /**
     * @return 门店详情
     */
    public function shopQueryDetail($data){
        $ss['content']['biz_content'] = json_encode($data);
        $ss['apiMethodName'] = "alipay.offline.market.shop.querydetail";
        $re = $this->execute($ss);
        return $re['alipay_offline_market_shop_querydetail_response'];
    }
    /**
     * @return 门店摘要信息批量查询接口
     */
    public function shopSummaryBatchQuery($data){
        $ss['content']['biz_content'] = json_encode($data);
        $ss['apiMethodName'] = "alipay.offline.market.shop.summary.batchquery";
        $re = $this->execute($ss);
        return $re['alipay_offline_market_shop_summary_batchquery_response'];
    }
    /**
     * @return 门店类目
     */
    public function shopCategoryQuery($data){
        $ss['content']['biz_content'] = json_encode($data);
        $ss['apiMethodName'] = "alipay.offline.market.shop.category.query";
        $re = $this->execute($ss);
        return $re['alipay_offline_market_shop_category_query_response'];
    }
